...
class CampeonatoEquipoRepo extends BaseRepo{
	...
	public function getEquiposInLiga($ligaId)
	{
	 	$ids = CampeonatoEquipo::whereHas('campeonato',function($q) use ($ligaId){
											$q->where('liga_id',$ligaId);
									})
									->select(\DB::raw('distinct campeonato_id'))
									->pluck('campeonato_id')
									->get();
		return Equipo::whereIn('id',$ids)->get();
	}
	...
	public function getEquiposWithPosicionesByCampeonatos($campeonatosId)
	{
		 $equiposCampeonato = CampeonatoEquipo::whereIn('campeonato_id',$campeonatosId)->with('equipo')->get();
		 $equipos = array();
		 foreach($equiposCampeonato as $equipo)
		 {
		 	$e = new EquipoPosicion($equipo->equipo);
		 	$equipos[$equipo->equipo->id] = $e;
		 }
		 return $equipos;
	}
